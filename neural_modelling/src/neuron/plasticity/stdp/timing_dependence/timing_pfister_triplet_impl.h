	...
	post_trace_t last_trace)
{
    ...
    // If this is the 1st post-synaptic event, o2 trace is zero (as it's
    // sampled BEFORE the spike), otherwise, add on energy caused by last
    // spike and decay that
	...
	pre_trace_t last_trace)
{
    ...
    // If this is the 1st pre-synaptic event, r2 trace is zero (as it's
    // sampled BEFORE the spike), otherwise, add on energy caused by last
    // spike and decay that
